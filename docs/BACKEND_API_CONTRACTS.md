...
- `[POST] /api/mobile/karir/toefl/[testId]/submit` — req `{ skill:"reading"|"listening"|"structure", answers:{[qNumber]: string|string[]}, routedToHard?:boolean }` (routedToHard hanya iBT_2026). res per format: 2026 → `{attemptId,skill,format,correct,total,routedToHard,cefr,tier:{label,tier}}`; ITP → `{attemptId,...,sectionScore}` (31-68); legacy → `{attemptId,...,sectionScore}` (0-30). `answers` di-key per **nomor soal**; multi-choice value = array of index-string.
- `[POST] /api/mobile/karir/ielts/[testId]/submit` — req `{ skill:"listening"|"reading", answers:{[qNumber]: string} }` → `{ attemptId, skill, correct, total, band }` (band 1.0-9.0). tf_ng value="true|false|not_given", mc value=index-string, fill_blank value=teks.
- Tiap submit **dipersist** ke riwayat (LangTestAttempt) + balikin `attemptId`.

### `[GET]` /api/mobile/karir/history — _Bearer_  ⟵ Riwayat Tes (baru)
- **query**: `system=TOEFL|IELTS` (opsional), `testId` (opsional), `limit=1..100` (default 50)
- **res**: `{ attempts:[{ id, system, format, testId, skill, correct, total, scoreKind:"band"|"cefr"|"itp_section"|"ibt_legacy_section", scoreValue:number, routedToHard, createdAt }] }` (terbaru dulu; untuk layar Riwayat + tren skor)
